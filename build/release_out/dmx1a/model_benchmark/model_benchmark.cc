...
#include "AVL.h"
#elif defined(IA700)
#include "tensorflow/lite/micro/ia700/debug_log_callback.h"
#include "tensorflow/lite/micro/ia700/xt_profiler.h"
#include "AVL.h"
...
constexpr int kKeywordModelTensorArenaSize = 2*1024 * 1024; // ~ 1MB
...
#define KN_GET_ISS_CYCLES_IF_CC_NO_FUNC(c,x,b,a)          x        
...
#if defined(HMD1A) || defined(HEMILITE)
 ...
 static int
	 generic_inferenc(tflite::MicroInterpreter* interpreter,  char* input_npy[],
			 uint32_t* cycle_count, tflite::MicroProfiler &profiler)
 {
  ...
  // TfLiteTensor* input = interpreter->inputs(0);
   ...
   int32_t ticks = 0;
         ...
         for (int iter = 0; iter < input_iteration; iter++) {
           ...
           // below is a wekws input feat, cache example custom by model
           for (int jj = 0; jj < input_size; jj++)
           {
             ...
             switch (jj) {
               ...
               case 0:
                 if (iter == 0)
                 {
                   memset(inputx->data.int8, 0, inputx->bytes);
                 }
...
#ifdef __XTENSA__
      printf("cycle: %d profiler ticks: %8d\n", *cycle_count, ticks);
#endif
           ...
           for (int jj = 0; jj < output_size; jj++) {
             ...
             if (output->type == kTfLiteInt8) {
               ...
               {
                 int pi = 0;
                 TfLiteTensor* cache = output;// interpreter->output(1);
                 //for (int ii = 0,  pi = 0; pi < 16 && ii < cache->bytes / sizeof(float); ii++)
                 {
                   //if (cache->data.f[ii] != 0.0)
                    // printf("%4d %.10f %4d\n", ii, cache->data.f[ii], pi++);
                 }
               }
 ...
 int tensor_arena_test(int arena_size, uint8_t **model_buf, int nModels)
 {
  ...
  static uint8_t var_arena[4096];
  tflite::MicroAllocator* ma = tflite::MicroAllocator::Create(var_arena, 4096);
  tflite::MicroResourceVariables* mrv = tflite::MicroResourceVariables::Create(ma, 20);
	...
	for (int midx = 0; midx < nModels; midx++)
	{
		...
		tflite::MicroInterpreter* interpreter2 = new tflite::MicroInterpreter(
			tflite::GetModel(model_buf[midx]), op_resolver, gpAllocator2, mrv, &profiler);
		//write at bottom

		TfLiteStatus allocate_status = interpreter2->AllocateTensors();

		
		generic_inferenc(interpreter2, NULL, NULL, profiler);
 ...
 multiple_models_size(char *model[], char *npy[], int num_model) //int argc, char *argv[])
 {
	 ...
	 uint32_t	 event_handle = profiler.BeginEvent("Initialize Interpreter");
   static uint8_t var_arena[4096];
   tflite::MicroAllocator* ma = tflite::MicroAllocator::Create(var_arena, 4096);
   tflite::MicroResourceVariables* mrv = tflite::MicroResourceVariables::Create(ma, 20);
	 tflite::MicroAllocator* gpAllocator =
		 tflite::MicroAllocator::Create(allocator_buffer, allocator_buffer_size);
	 ...
	 for (int ii = 0; ii < num_model && ii < MAX_MODEL_NUM; ii++)
	 {
		 ...
		 interpreter[idx] = new tflite::MicroInterpreter(
			 tflite::GetModel(pKwModel[ii]), ops_resolver, gpAllocator, mrv, &profiler
			 );
		...
		if (stream)
		{
			...
			generic_inferenc(interpreter[idx], npy, &cycles_count[idx], profiler);
		}
