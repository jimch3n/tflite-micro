...
// void Crc32Table(uint32_t * table, size_t len,  uint32_t polynomial) {
//   uint32_t remainder;
//   for (int i = 0; i < len; i++) {
//     remainder = i;
...
//   }
// }
// and generate the first 256 elements: Crc32Table(table, 256, kCrc32Polynomial)
...
uint32_t Crc32Bitwise(const void *array, size_t len) {
  ...
  for (int byte_index = 0; byte_index < (int)len; byte_index++) {
    uint32_t byte = ptr[byte_index];
    crc = crc ^ byte;
