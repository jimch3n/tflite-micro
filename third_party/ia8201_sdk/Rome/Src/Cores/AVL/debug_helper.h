...
#define KN_PRINTACPX(FTL) do {\
   AComplex *tmpAflt =(AComplex *) &(FTL);\
   printf("%-40s: %10d %.10f%c%.10f\n",#FTL,__LINE__, (double)tmpAflt->r().to_double(),\
tmpAflt->i() >=CONST_ASCALAR(0) ? '+':'-', (double)tmpAflt->i().abs().to_double() ); \
  } while (0)
#define KN_PRINTACPX_G(FTL) do {\
   AComplex *tmpAflt =(AComplex *) &(FTL);\
   printf("%-40s: %10d %g%c%g\n",#FTL,__LINE__, (double)tmpAflt->r().to_double(),\
tmpAflt->i() >=CONST_ASCALAR(0) ? '+':'-', (double)tmpAflt->i().abs().to_double() ); \
  } while (0)
