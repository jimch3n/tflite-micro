/************************************************************************//**
 * @file    AVL/DMX1A/AScalar.h
 *
 * @brief   AScalar wrapper for fr32 on DMX1A
 *
...
/**************************************************************************//**
 * @defgroup dmx1a_avl DMX1A Audience Vector Library 
 * @{
 *****************************************************************************/
...
public:
    ...
    _AI int to_int(uint32_t fractional_bits = 0) const {
        ...
        return (int16_t)((move32_ar_vr(v, VRQ0)&0xffff0000)>>16); // get highpart
    }
    _AI int fr_exp() const {
        vr128 v,e ;
        set_VRQ0(v, fr);
        e = afloat_exp_extract(v);
        return move32_ar_vr(e, VRQ0);
    }
    _AI int floor() const {
    ...
    _AI AScalar f_log10() const { return fpmul(log2_fr(fr), log10two().fr, 0); }
    ...
    _AI static AScalar epsilon() { return seta_fr(kConstTable_Epsilon, 0, 0); }
...
/** \class AScalar
    ...
    \brief Returns sin(2p * (*this))

    \fn AScalar AScalar::cosine_turns()
    \brief Returns cos(2p * (*this))
