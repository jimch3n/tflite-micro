...
#ifndef __CSTUB_Xm_hemilite_shapiro_RI04_C_H__
#define __CSTUB_Xm_hemilite_shapiro_RI04_C_H__

/* Included files */
#include "cstubs_include/hemilite_common_tie_cstubs.h"
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
...
#pragma warning( disable : 4047 4100 4244)
#if defined(_WIN64) 
#error "Error: Compiling c-stub on 64 bit machines is not supported"
#endif /* _WIN64 */
...
typedef struct CSTUB_(processor) {
  ...
  CSTUB_(InputQueue_Internal_t) IPQ;
  unsigned cstub_IPQ_data[1];
  CSTUB_(OutputQueue_Internal_t) OPQ;
...
#define table__bit_mask CSTUB_(table__bit_mask)
extern const unsigned int table__bit_mask[32]; 
#define table__vmac_sel CSTUB_(table__vmac_sel)
extern const unsigned int table__vmac_sel[16]; 
#define table__blender CSTUB_(table__blender)
extern const unsigned int table__blender[16]; 
#define table__set_values CSTUB_(table__set_values)
extern const unsigned int table__set_values[32]; 
#define table__nonlinear_a0 CSTUB_(table__nonlinear_a0)
extern const unsigned int table__nonlinear_a0[256]; 
#define table__nonlinear_a1 CSTUB_(table__nonlinear_a1)
extern const unsigned int table__nonlinear_a1[256]; 
#define table__nonlinear_a2 CSTUB_(table__nonlinear_a2)
extern const unsigned int table__nonlinear_a2[256]; 
...
typedef union CSTUB_(mem) {
  ...
  unsigned int   d1024[32];
} CSTUB_MAY_ALIAS CSTUB_(mem_t);

static INLINE void MemDataIn1024(unsigned addr, unsigned opcode, unsigned *data) {
...
static INLINE void MemDataOut1024(unsigned addr, const unsigned *data, const unsigned *disables, unsigned opcode) {
  CSTUB_(mem_t) *dst = (CSTUB_(mem_t) *) addr;
  const unsigned *src = data;
  if ((disables[0] == 0) && (disables[1] == 0) && (disables[2] == 0) && (disables[3] == 0)) {  
    dst->d1024[0] = src[0];
    dst->d1024[1] = src[1];
    dst->d1024[2] = src[2];
    dst->d1024[3] = src[3];
    ...
    dst->d1024[30] = src[30];
    dst->d1024[31] = src[31];
  } else  if ((disables[0] == 0xffffffff) && (disables[1] == 0xffffffff) && (disables[2] == 0xffffffff) && (disables[3] == 0xffffffff)) {
    /* no store */
  } else {
    dst->d1024[0] = (dst->d1024[0] & (~mask[(disables[0] >> 0) & 0xf])) | (src[0] & mask[(disables[0] >> 0) & 0xf]);
    dst->d1024[1] = (dst->d1024[1] & (~mask[(disables[0] >> 4) & 0xf])) | (src[1] & mask[(disables[0] >> 4) & 0xf]);
    dst->d1024[2] = (dst->d1024[2] & (~mask[(disables[0] >> 8) & 0xf])) | (src[2] & mask[(disables[0] >> 8) & 0xf]);
    dst->d1024[3] = (dst->d1024[3] & (~mask[(disables[0] >> 12) & 0xf])) | (src[3] & mask[(disables[0] >> 12) & 0xf]);
    dst->d1024[4] = (dst->d1024[4] & (~mask[(disables[0] >> 16) & 0xf])) | (src[4] & mask[(disables[0] >> 16) & 0xf]);
    dst->d1024[5] = (dst->d1024[5] & (~mask[(disables[0] >> 20) & 0xf])) | (src[5] & mask[(disables[0] >> 20) & 0xf]);
    dst->d1024[6] = (dst->d1024[6] & (~mask[(disables[0] >> 24) & 0xf])) | (src[6] & mask[(disables[0] >> 24) & 0xf]);
    dst->d1024[7] = (dst->d1024[7] & (~mask[(disables[0] >> 28) & 0xf])) | (src[7] & mask[(disables[0] >> 28) & 0xf]);
    dst->d1024[8] = (dst->d1024[8] & (~mask[(disables[1] >> 0) & 0xf])) | (src[8] & mask[(disables[1] >> 0) & 0xf]);
    dst->d1024[9] = (dst->d1024[9] & (~mask[(disables[1] >> 4) & 0xf])) | (src[9] & mask[(disables[1] >> 4) & 0xf]);
    dst->d1024[10] = (dst->d1024[10] & (~mask[(disables[1] >> 8) & 0xf])) | (src[10] & mask[(disables[1] >> 8) & 0xf]);
    dst->d1024[11] = (dst->d1024[11] & (~mask[(disables[1] >> 12) & 0xf])) | (src[11] & mask[(disables[1] >> 12) & 0xf]);
    dst->d1024[12] = (dst->d1024[12] & (~mask[(disables[1] >> 16) & 0xf])) | (src[12] & mask[(disables[1] >> 16) & 0xf]);
    dst->d1024[13] = (dst->d1024[13] & (~mask[(disables[1] >> 20) & 0xf])) | (src[13] & mask[(disables[1] >> 20) & 0xf]);
    dst->d1024[14] = (dst->d1024[14] & (~mask[(disables[1] >> 24) & 0xf])) | (src[14] & mask[(disables[1] >> 24) & 0xf]);
    dst->d1024[15] = (dst->d1024[15] & (~mask[(disables[1] >> 28) & 0xf])) | (src[15] & mask[(disables[1] >> 28) & 0xf]);
    dst->d1024[16] = (dst->d1024[16] & (~mask[(disables[2] >> 0) & 0xf])) | (src[16] & mask[(disables[2] >> 0) & 0xf]);
    dst->d1024[17] = (dst->d1024[17] & (~mask[(disables[2] >> 4) & 0xf])) | (src[17] & mask[(disables[2] >> 4) & 0xf]);
    dst->d1024[18] = (dst->d1024[18] & (~mask[(disables[2] >> 8) & 0xf])) | (src[18] & mask[(disables[2] >> 8) & 0xf]);
    dst->d1024[19] = (dst->d1024[19] & (~mask[(disables[2] >> 12) & 0xf])) | (src[19] & mask[(disables[2] >> 12) & 0xf]);
    dst->d1024[20] = (dst->d1024[20] & (~mask[(disables[2] >> 16) & 0xf])) | (src[20] & mask[(disables[2] >> 16) & 0xf]);
    dst->d1024[21] = (dst->d1024[21] & (~mask[(disables[2] >> 20) & 0xf])) | (src[21] & mask[(disables[2] >> 20) & 0xf]);
    dst->d1024[22] = (dst->d1024[22] & (~mask[(disables[2] >> 24) & 0xf])) | (src[22] & mask[(disables[2] >> 24) & 0xf]);
    dst->d1024[23] = (dst->d1024[23] & (~mask[(disables[2] >> 28) & 0xf])) | (src[23] & mask[(disables[2] >> 28) & 0xf]);
    dst->d1024[24] = (dst->d1024[24] & (~mask[(disables[3] >> 0) & 0xf])) | (src[24] & mask[(disables[3] >> 0) & 0xf]);
    dst->d1024[25] = (dst->d1024[25] & (~mask[(disables[3] >> 4) & 0xf])) | (src[25] & mask[(disables[3] >> 4) & 0xf]);
    dst->d1024[26] = (dst->d1024[26] & (~mask[(disables[3] >> 8) & 0xf])) | (src[26] & mask[(disables[3] >> 8) & 0xf]);
    dst->d1024[27] = (dst->d1024[27] & (~mask[(disables[3] >> 12) & 0xf])) | (src[27] & mask[(disables[3] >> 12) & 0xf]);
    dst->d1024[28] = (dst->d1024[28] & (~mask[(disables[3] >> 16) & 0xf])) | (src[28] & mask[(disables[3] >> 16) & 0xf]);
    dst->d1024[29] = (dst->d1024[29] & (~mask[(disables[3] >> 20) & 0xf])) | (src[29] & mask[(disables[3] >> 20) & 0xf]);
    dst->d1024[30] = (dst->d1024[30] & (~mask[(disables[3] >> 24) & 0xf])) | (src[30] & mask[(disables[3] >> 24) & 0xf]);
    dst->d1024[31] = (dst->d1024[31] & (~mask[(disables[3] >> 28) & 0xf])) | (src[31] & mask[(disables[3] >> 28) & 0xf]);
  }
}
...
static INLINE void MemDataOut1024WODisable(unsigned addr, const unsigned *data, unsigned opcode) {
  CSTUB_(mem_t) *dst = (CSTUB_(mem_t) *) addr;
  const unsigned *src = data;
  dst->d1024[0] = src[0];
  dst->d1024[1] = src[1];
