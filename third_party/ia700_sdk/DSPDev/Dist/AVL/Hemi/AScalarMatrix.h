    ...
    void transpose(const AScalarMatrix& x) {
        ...
        for (uint16_t i = 0; i < rows; i++) {
            ...
            if (cols & 1) {
                ...
                #endif
                load32x1_vr_idxI(V_dst, (const unsigned int*)P_src, IDX0, VRQ0);
                store32x1_vr_idxI(V_dst, (unsigned int*)SP_dst.ptr, IDX0, VRQ0);
            }
        }
    }
        ...
        for (uint16_t i = 0; i < rows; i++) {
            for (uint16_t j = 0; j < cols; j++) {
                ...
                if (cols & 1) {
                    ...
                    vr64 V_src_x=vseta_vr(0,0);
                    vr64 V_src_y=V_src_x;
                    load32x1_vr_idxI(V_src_x, (const unsigned int*)LP_src_x.ptr, IDX0, VRQ0);
                    load32x1_vr_idxI(V_src_y, (const unsigned int*)LP_src_y.ptr, IDX0, VRQ0);
