...
* $Revision: #1 $
* $Id: //AudEngr/AlgoSW/Product/sdk-support-TFuL-dev/Rome/Src/Packages/FrontEnd/ksp_frontend/generic/inc/ksp_math.h#1 $
*
* @copyright
...
{
#if defined(AVL_DELTAMAX) || defined (AVL_HEMI) || defined(AVL_DMX1A)
...
{
    ksp_float_reg_t twentyInverse = CONST_ASCALAR(1.0 / 20.0);
...
extern "C" {
    ...
    ****************************************************************************/
	...
	/*****************************************************************************
	* ksp_math_sqrt_inner_prod_real
	*************************************************************************//**
	* Square-Root Inner product of two real vectors.
	*
    * *pOut = sqrt(px[0]*py[0]+px[1]*py[1]+....+px[len-1]*py[len-1])
	*
	* @param[in]   *px             Vector for inner product
	* @param[in]   *py             Vector for inner product
	* @param[in]   len             Length of the vector for inner product
	* @param[out]  *pOut           Pointer to output of square-root inner product
	*							   *pOut = sqrt(px[0]*py[0]+px[1]*py[1]+....+px[len-1]*py[len-1])
	* @returns     status          Returns KSP_SUCCESS or ksp_status_t type value
	for the error.
	****************************************************************************/
	ksp_status_t  ksp_math_sqrt_inner_prod_real(
		const ksp_float_t     *px,
		const ksp_float_t     *py,
		ksp_int32_t            len,
		ksp_float_t           *pOut
