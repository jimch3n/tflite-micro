...
#ifndef BOSKO_DEBUG_HELPER_H_
#define BOSKO_DEBUG_HELPER_H_
...
#if defined( KN_DEBUG) && !defined(__XTENSA__)
#include <stdio.h>
...
#define KN_PRINTACMPX(CMPX) do {\
   AComplex *tmpAflt =(AComplex *) &(CMPX);\
   printf("%s = %10d %.10f + %.10fj\n",#CMPX,__LINE__, (double)tmpAflt->r().to_double(), (double)tmpAflt->i().to_double()); \
  } while (0)
...
#define KN_PRINT_VR64(VR_IN) do {\
	VAR_ALIGN_8 AComplex temp_dmp;\
	store32x2_vr_idxI(VR_IN,(unsigned int*) &temp_dmp, 0);\
	printf("%-16s: %d\t0: %.11f\t1: %.11f\n", \
    #VR_IN, __LINE__, \
	(double)temp_dmp.r().to_float(),\
(double)temp_dmp.i().to_float());\
}while(0)
...
#define KN_PRINT_AFLOAT16(IN,  SIZE) do {\
	...
	for(int ii = 0; ii< (int)SIZE; ii++){\
    vr64 vr_flt ; load16x1_vr_postI(vr_flt, pTmp, INC1, VRQ0);\
