...
void AffineQuantizeFloat32ToInt8(struct OpData* op_data,
  ...
  vr128 VR_zeroPoint;
  vr128 VR_out, VR_q7_out, vr_output;
  ulsr128 UR_dataIn = align_32x4_load(pInput);
  ulsr128 UR_out = align_8x4_store(pOut);
  replicate_ar(VR_scale, 0xf, op_data->inv_scale.fr);
...
void ReQuantizeFloat32ToFloat16(struct OpData* op_data,
  ...
  if (loopLim > 0) {
    ...
    for (int i = 0; i < loopLim - 1; i++) {
      convert_IEEE_float_to_32F_x4(VR_dataIn);
      convert_32F_to_16F_x4(VR_dataIn, TF_FLT16_SIGN, TF_FLT16_EXP,
                            TF_FLT16_BIAS, 1);
      store_16x4_vr_a(VR_dataIn, UR_out, pOut);
      // store32x1_vr_postI(VR_q7_out, pDst, INC1, VRQ0);
      load_32x4_vr_a(VR_dataIn, UR_dataIn, pInput);
    }
    convert_IEEE_float_to_32F_x4(VR_dataIn);
    convert_32F_to_16F_x4(VR_dataIn, TF_FLT16_SIGN, TF_FLT16_EXP, TF_FLT16_BIAS,
                          1);
    store_16x4_vr_a(VR_dataIn, UR_out, pOut);
    flush_16x4(UR_out, pOut);
  }
  // TODO:
  if (flat_size & 3) {
    for (int ii = 0; ii < (flat_size & 3); ii++) {
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (output->type != kTfLiteFloat16) {  // skip float16 output
