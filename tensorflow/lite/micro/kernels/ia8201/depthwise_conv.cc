...
#ifdef DMX1A_DW_CONV_OPT
static VAR_ALIGN_16 uint32_t tempVB[4 * 4];
static VAR_ALIGN_16 unsigned short bytearray_dmx[] = {
...
int DepthWiseConvKernel8xnInputOffset(const int32_t inputOffset,
  ...
  uint32_t *pBitArrays2 = (uint32_t *)bytearray_dmx;

  // vr128 VR_yTmp;
  vr128 VR_inputOffset;
  vr128 VR_x;  // , VR_x2;
  vr128 VR_acc, VR_coeff;
  xtbool2 signSpec = int_to_xt_bool2(multSigns);
  // ulsr128 UR_x;
  int32_t *pA1 = (int32_t *)pAinput;
  ulsr128 UR_A = align_32x4_load(pA1);
  ...
  for (int jj = 0; jj < n8; jj++) {
    ...
    store32x4_vr_postI(VR_inputOffset, pTempVBPtr, INC1);
    ...
    // SKIP VB3?
    load_VB4_postI(pTempVBPtr, INC1);

    VR_x = vexp_adji(VR_inputOffset, 0);
    WUR_MvmAux(0);
    // FIX BUG:
    pBitArrays2 = (uint32_t *)bytearray_dmx;
    ...
    pBitArrays2 = (uint32_t *)bytearray_dmx;
    load_BB4_postI(pBitArrays2, INC1);
    load_BB5_postI(pBitArrays2, INC1);
    load_BB6_postI(pBitArrays2, INC1);
    load_BB7_postI(pBitArrays2, INC1);

    load_32x4_vr_a(VR_coeff, UR_A, pA1);
    // 16*3
    for (int i = 0; i < loopCount - 1; i++) {
      // mac8bx8b_spare_SIM_DMX_debug(&VR_acc, &VR_coeff, &VR_x, multSigns);
  ...
  // if(filter_size&1)
  {
    replicate_ar(VR_inputOffset, 0xc, 0);  // SET High part Inputoffset zero
    for (int i = 0; i < (n & 7); i++) {
      WUR_MvmAux(0);
      set_BB_zero();
      // load32x2_vr_postR(VR_x, pX, m2, VRL);  // load column 8
      pBitArrays2 = (uint32_t *)bytearray_dmx;
      load_BB0_postI(pBitArrays2, INC1);
      load_BB1_postI(pBitArrays2, INC1);
      load_BB2_postI(pBitArrays2, INC1);
...
static int DepthWiseConvFloat16Kernel(float *x, const TfLiteFloat16 *A,
  ...
  for (int i = 0; i < m; i++) {
    VR_y = vseta_vr(0, 0, 0);
    vr128 VR_out, VR_bias;
    pX = x;
    ulsr128 UR_A = align_16x4_load(pA);
    ulsr128 UR_x = align_32x4_load(pX);
    load_16x4_vr_a(VR_A, UR_A, pA);
    load_32x4_vr_a(VR_x, UR_x, pX);

    for (int j = 0; j < loopLimCol - 1; j++) {
      convert_IEEE_float_to_32F_x4(VR_x);
      convert_16F_to_32F_x4(VR_A, TF_FLT16_SIGN, TF_FLT16_EXP, TF_FLT16_BIAS);

      VR_y = vmacs_adj(VR_y, VR_x, VR_A, 0, 0);  // linear
                                                 //  KN_PRINT_VR(VR_y);
      load_16x4_vr_a(VR_A, UR_A, pA);
      load_32x4_vr_a(VR_x, UR_x, pX);
    }
    ...
    VR_y = vmacs_adj(VR_y, VR_x, VR_A, 0, 0);
    for (int ii = 0; ii < remainCol; ii++) {
...
static int DepthWiseConvFloatInt8Kernel(float *x, const int8_t *A,
  ...
  for (int i = 0; i < m; i++) {
    ...
    for (int j = 0; j < loopLimCol - 1; j++) {
      convert_IEEE_float_to_32F_x4(VR_x);
      convert_16I_to_32F_x4(VR_A, wBlkExp);
    ...
    VR_out = vmuls(VR_out, VR_scale, 0);
    //    KN_PRINT_VR(VR_out);
    //    KN_PRINT_VR(VR_bias);
...
static int DepthWiseConvFloatKernel(float *x, const float *A, const float *bias,
  ...
  for (int i = 0; i < m; i++) {
    ...
    for (int j = 0; j < loopLimCol - 1; j++) {
      convert_IEEE_float_to_32F_x4(VR_x);
      convert_IEEE_float_to_32F_x4(VR_A);
...
static int DepthWiseConvFloatInt8KernelVIP(float *x, const int8_t *A,
  ...
  vr128 VR_scale;
  int nextIn = m >> 2;
  TF_LITE_ASSERT((m & 3) == 0);

  ulsr128 UR_b = align_32x4_load(pBias);
  ...
  if (loopLimRow > 0) {
    for (int i = 0; i < loopLimRow; i++) {
      const float *pX = x + i * 4;
      const int8_t *pA = (int8_t *)A + i * 4 * n;
      ...
      load32x4_vr_postR(VR_x, pX, nextIn);
      ...
      load_32x4_vr_a(VR_bias, UR_b, pBias);
      convert_IEEE_float_to_32F_x4(VR_bias);
      ...
      convert_32F_to_IEEE_float_x4(VR_out);
      store_32x4_vr_a(VR_out, UR_out, pDst);
    }

    flush_32x4(UR_out, pDst);
  }
  return 0;
}
...
static int DepthWiseConvFloat16KernelVIP(float *x, const TfLiteFloat16 *A,
  ...
  replicate_ar(VR_min, 0xf, act_min.fr);
  {
    for (int i = 0; i < loopLimRow; i++) {
      const float *pX = x + i * 4;
      const TfLiteFloat16 *pA = (TfLiteFloat16 *)A + i * 4 * n;
...
static int DepthWiseConvFloatKernelVIP(float *x, const float *A,
  ...
  //  for(int g = 0; g < loopLimRow; g++)
  {
    ...
    for (int i = 0; i < loopLimRow; i++) {
      ...
      load_32x4_vr_a(VR_A, UR_A, pA);  //, nextIn);
      load32x4_vr_postR(VR_x, pX, nextIn);

      for (int j = 0; j < n - 1; j++) {
        convert_IEEE_float_to_32F_x4(VR_x);
        convert_IEEE_float_to_32F_x4(VR_A);
        //     if (dbg_enable){  KN_PRINT_VR(VR_A);      KN_PRINT_VR(VR_x);}

        VR_y = vmacs_adj(VR_y, VR_x, VR_A, 0,
                         0);  // linear
                              //  if (dbg_enable){  KN_PRINT_VR(VR_y); }
        load_32x4_vr_a(VR_A, UR_A, pA);  //, nextIn);
        load32x4_vr_postR(VR_x, pX, nextIn);
      }
      ...
      load_32x4_vr_a(VR_bias, UR_b, pBias);  //, INC1);
      convert_IEEE_float_to_32F_x4(VR_bias);
      // dsums_L(VR_y, VR_y, 0, 0);
      // fadds(VR_out, VRQ0, VR_y, VRQ0, VR_y, VRQ1, 0);
      VR_out = vadds(VR_y, VR_bias, 0);

      // if (dbg_enable){  KN_PRINT_VR(VR_out);KN_PRINT_VR(VR_bias);}
      // fadds(VR_out, VRQ0, VR_out, VRQ0, VR_bias, VRQ0, 0);
      ...
      //  KN_PRINT_VR(VR_out);
      convert_32F_to_IEEE_float_x4(VR_out);
      store_32x4_vr_a(VR_out, UR_out, pDst);  //, INC1, VRQ0);
    }
  }
  flush_32x4(UR_out, pDst);
  return 0;
...
int DepthWiseConvKernel8xn(const int32_t *x, const int32_t *pAinput,
  ...
  if (inputOffsetWithW == nullptr) {
    ...
    load32x4_vr_postI(VR_acc, inputOffsetW, INC1);
    load32x4_vr_postI(VR_x0, inputOffsetW, INC1);
  ...
  for (int jj = 0; jj < n8; jj++) {
    ...
    // FIX BUG:
    pTempVBPtr = tempVB;
    load32x2_vr_postR(VR_yTmp, pX, m2,
                      VRL);  // load Y Temp, move in the loop boday

    load32x2_vr_postR(VR_yTmp, pX, m2, VRH);  // VR16

    // split into 32x2, 32x2..
    load32x2_vr_postR(VR_x0, pX, m2, VRL);
    load32x2_vr_postR(VR_x0, pX, m2, VRH);
    store32x4_vr_postI(VR_x0, pTempVBPtr, INC1);
...
int DepthWiseConvApplyOffsetPerCh(
  ...
  for (uint32_t group = 0; group < groupOfChannel; group++) {
    ...
    // accExt
    // for not multiple 4 alignment issue.
    // if ((output_depth & 3) == 0) {
...
int DepthWiseConvSparseInt8PerCh(
  ...
  for (int g = 0; g < (m >> 3); g++) {
    int32_t *scratchPerGroup = pScratch + 8 * g;
    int8_t *pBuffer = (int8_t *)x + (g)*8;  // +f*nPerFilter*channel;
    int8_t *pKernel = (int8_t *)AInput + 8 * filter_dim_align2 * g;
    int32_t *inputOffsetGroup =
        (inputOffsetW == nullptr) ? 0 : inputOffsetW + g * 8;
...
// DepthWiseConvQuantizedInt8PerCh
int DepthWiseConvQuantizedInt8PerChInputOffset(
  ...
  // vr128 VR_outOffset;
  // vr128 VR_inputOffset;
  // replicate_ar(VR_inputOffset, 0xf, input_offset);  // -1 ~ -127
  // replicate_ar(VR_outOffset, 0xf, outOffsetFr32.fr);
...

{
  ...
  if (data_ex->opt_constraint == DEPTHWISE_CONV_OPT_MAC8Bx8B) {
    ...
    // inFCM = ch_align;                           //
    inFCN = ds_conv2d.ker_x * ds_conv2d.ker_y *
            ds_conv2d.in_ch;  // filter_dim without ch
    int inFCNA4 = (((inFCN + 3) >> 2) << 2);
    inFCN16Size = ((inFCNA4) + 15) / 16;
