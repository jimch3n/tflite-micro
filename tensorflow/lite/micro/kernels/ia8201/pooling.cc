...
#ifdef DMX1A_POOL_OPT
static VAR_ALIGN_16 const unsigned short bytearray_dmx[] = {
...
int AvgPoolQuantizedKernelInt8(const OpData &data, const int32_t *x,
  ...
  int m2 = (depth + 7) >> 3;  // 32x2 8

  int32_t *pDst = pScratchOuput;
  uint32_t *pBitArrays2 = (uint32_t *)bytearray_dmx;

  vr128 VR_out, VR_invFilterCount;
  vr128 VR_const1;  // , VR_x2;
  vr128 VR_acc, VR_coeff;
  xtbool2 signSpec = int_to_xt_bool2(sign);
  int exp_fxp = (sign == 2) ? 31 - 1 : 31 - 2;

  int n8 = (n / depth) >> 3;
  int n8R = (((n / depth) & 7));

  replicate_ar(VR_coeff, 0xf, 0x01010101);

  // KN_PRINTD(m2);

  // KN_PRINTD(n8);KN_PRINTD(n8R);
  set_BB_zero();
  VR_const1 = load32x4_vr_idxI(pTempVBPtr, IDX0);
  ...
  for (int jj = 0; jj < m2; jj++) {
    ...
    for (int ii = 0; ii < n8; ii++) {
      vr128 VR_coeff2, VR_coeff3, VR_coeff4;
      // pTempVBPtr = tempVB;
      ...
      // 16*4 = 64
      load32x2_vr_postR(VR_coeff, pX, m2,
                        VRL);  // load Y Temp, move in the loop boday
      load32x2_vr_postR(VR_coeff, pX, m2, VRH);  // VR16

      mac8bx8b_sparse(VR_acc, VR_coeff, VR_const1, signSpec);
      load32x2_vr_postR(VR_coeff2, pX, m2,
                        VRL);  // load Y Temp, move in the loop boday
      load32x2_vr_postR(VR_coeff2, pX, m2, VRH);  // VR16

      mac8bx8b_sparse(VR_acc, VR_coeff2, VR_const1, signSpec);

      load32x2_vr_postR(VR_coeff3, pX, m2,
                        VRL);  // load Y Temp, move in the loop boday
      load32x2_vr_postR(VR_coeff3, pX, m2, VRH);  // VR16

      mac8bx8b_sparse(VR_acc, VR_coeff3, VR_const1, signSpec);

      load32x2_vr_postR(VR_coeff4, pX, m2,
                        VRL);  // load Y Temp, move in the loop boday
      load32x2_vr_postR(VR_coeff4, pX, m2, VRH);  // VR16

      mac8bx8b_sparse(VR_acc, VR_coeff4, VR_const1, signSpec);
    }
    VR_coeff = vseta_vr(0, 0, 0);
    for (int kk = 0; kk < n8R; kk++) {
      WUR_MvmAux(0);

      set_BB_zero();
      load32x2_vr_postR(VR_coeff, pX, m2, VRL);  // load column 8
      pBitArrays2 = (uint32_t *)bytearray_dmx;
      load_BB0_postI(pBitArrays2, INC1);
      load_BB1_postI(pBitArrays2, INC1);
      load_BB2_postI(pBitArrays2, INC1);
  ...
  replicate_ar(VR_invFilterCount, 0xf, inverse_n.fr);
  ...
  if (m4 > 0) {
    ...
    for (int jj = 0; jj < m4; jj++) {
      ...
      convert_32F_to_16I_x4(VR_out, 15, 1);          // rounding
      VR_out = shift32_arith(VR_out, (unsigned)-16, 0);

      SATURATE_INT32_VR128(VR_out, VR_out, data.activation_min,
                           data.activation_max);
  ...
  for (int jj = 0; jj < (depth & 3); jj++) {
    ...
    // load32x1_vr_postI(VR_acc, pDst, INC1);
  }
...
int MaxPoolKernelQuantizedUInt8(
  ...
  replicate_ar(VR_mask, 0xf, 0x00ff0000);
#ifdef KN_DEBUG
  CHECK_ALIGN_4(pData);
#endif
  ...
  for (int jj = 0; jj < m2; jj++) {
    pData = pDataGroup;
    load8x4_vr_postR(VR_data, pData, depthInc);
...

{
  ...
  // int inFCN8 = conv2d.ker_x * conv2d.ker_y * inFCMA8;
  ...
  for (int i_out_y = 0; i_out_y < conv2d.out_y; ++i_out_y) {
    for (int i_out_x = 0; i_out_x < conv2d.out_x; ++i_out_x) {
      ...
      //  tflite::im2col_padding(pIm2ColBuf + offset_im_dst,
...
#ifdef HMD1A_POOL_OPT
static VAR_ALIGN_16 const unsigned int bytearray_hmd[] = {
