...
TfLiteStatus CreateGateParams(
  ...
  // 
   ...
   gate_params.input_fc_params = FullyConnectedParamsQuantized(fc_data_temp_ex.FcOp);
   ...
   if (opt_constraint) {
    ...
    KN_PRINTD(m_input);  KN_PRINTD(n_input);
    

    
    int input_idx =    lstm_tensors.GetTensorIndex(input_weight);
    const int32_t input_coeff_size = tflite::FullyConnectedMap8bitCoeffs(
        NULL, NULL, m_input, n_input);

      const TfLiteEvalTensor *evalInputTensor = tflite::micro::GetEvalInput(context, node,input_idx );  
      const int8_t *input_w = tflite::micro::GetTensorData<int8_t>(evalInputTensor);

      // bias if avaible;
      int inputb_idx =  lstm_tensors.GetTensorIndex(input_bias);
      KN_PRINTD(inputb_idx);
      const TfLiteEvalTensor *evalBiasTensor = tflite::micro::GetEvalInput(context, node,inputb_idx );  
      const int32_t *pB = tflite::micro::GetTensorData<int32_t>(evalBiasTensor);

      if( !tflite::is_coeffs_mapped(context) ) // get  tflite::is_coeffs_mapped(context) == true
      {
        
        // allocate new persist tensor data
      p_input_mapped_filter =
          (int32_t *)context->AllocatePersistentBuffer(context, input_coeff_size);
      if (p_input_mapped_filter) {
        ...
        KN_PRINT_Q7_SIZE_ATMOST(p_input_mapped_filter, 
        m_input*n_input, 64);
    ...
    fc_data_temp_ex.input_offset_int8 = (offsetInput << 24) | (offsetInput << 16) |
  ...
  TF_LITE_ENSURE_STATUS(CalculateOpDataFullyConnected(
      ...
      hidden_state_weight, hidden_state_bias, &fc_output_temp, &fc_data_temp_ex));
   ...
   if (opt_constraint) {
    ...
    RuntimeShape hidden_state_weight_shape =  GetTensorShape(hidden_state_weight);

    
    int n_hidden = hidden_state_weight_shape.Dims(1);
    int m_hidden = hidden_state_weight_shape.Dims(0);

    KN_PRINTD(n_hidden);  KN_PRINTD(m_hidden);
    
    int hidden_idx =  lstm_tensors.GetTensorIndex(hidden_state_weight);

    const int32_t hidden_coeff_size = tflite::FullyConnectedMap8bitCoeffs(
    NULL, NULL, m_hidden, n_hidden);
      const TfLiteEvalTensor *evalHiddenTensor = tflite::micro::GetEvalInput(context, node,hidden_idx );
      
  const int8_t *hidden_w = tflite::micro::GetTensorData<int8_t>(evalHiddenTensor);

  
      int hiddenb_idx =  lstm_tensors.GetTensorIndex(hidden_state_bias);
      KN_PRINTD(hiddenb_idx);
      const TfLiteEvalTensor *evalHBiasTensor = tflite::micro::GetEvalInput(context, node,hiddenb_idx );
...
TfLiteStatus PrepareGateParametersInteger(TfLiteContext* context,
  ...
  int opt_constraint = activation_type == kTfLiteInt8 && 
  weight_type == kTfLiteInt8; 
#else
  int opt_constraint =0;
#endif
  float nonlinear_input_scale = 0.000244140625;  // 2^-12 Q3.12 -> Q0.15
  KN_PRINTS("forget gate param");
  TF_LITE_ENSURE_OK(
      context,
      CreateGateParams(
          context, node, lstm_tensors, lstm_tensors.GetInternalTensor(kLstmInputTensor),
  ...
  op_data_lstm->inter_gate_parameters.output_mul_params =
      CreateInterGateMulParams(
          ...
          lstm_tensors.HiddenStateTensor()->params.zero_point);
              KN_PRINTS("=== end prepareGateParamInt");
  return kTfLiteOk;
}
