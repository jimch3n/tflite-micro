...
void *Init(TfLiteContext *context, const char *buffer, size_t length) {
  ...
  return context->AllocatePersistentBuffer(context, sizeof(OpDataFullyConnectedEx));
}
...
int FullyConnectedFloatInt8(float* y, const float* x, const int8_t* A,
    ...
    if (loopGroupLim > 0) {
        for (jj = 0; jj < loopGroupLim; jj++)  // group
        {
            ...
            if (loopBlockLim2 > 0) {
                ...
                affine_adj_L(VR_out0, VR_w0, VR_blk, 0, 0);
                //   KN_PRINT_VR128(VR_out0);
                load_8x4_vr_a(VR_w0, UR_w1, pWa1);convert_16I_to_32F_x4(VR_w0, wBlkExp);
            
                affine_adj_H(VR_out0, VR_w0, VR_blk, 0, 0);
                load_8x4_vr_a(VR_w0, UR_w2, pWa2);convert_16I_to_32F_x4(VR_w0, wBlkExp);
            
                affine_adj_L(VR_out1, VR_w0, VR_blk, 0, 0);
                load_8x4_vr_a(VR_w0, UR_w3, pWa3);convert_16I_to_32F_x4(VR_w0, wBlkExp);
            ...
            if (n & 3) {
                ...
                for (ii = loopBlockLim2 << 2; ii < n; ii++) {
                    ...
                    load8x1_vr_postI(VR_w0, pWa1, INC1, VRQ0);convert_16I_to_32F_x1(VR_w0,wBlkExp, VRQ0);
             
                    fmacs(VR_out0, VRQ2, VR_w0, VRQ0, VR_blk, VRQ0, 0);

                    load8x1_vr_postI(VR_w0, pWa2, INC1, VRQ0);convert_16I_to_32F_x1(VR_w0, wBlkExp, VRQ0);
   
                    fmacs(VR_out1, VRQ0, VR_w0, VRQ0, VR_blk, VRQ0, 0);

                    load8x1_vr_postI(VR_w0, pWa3, INC1, VRQ0);convert_16I_to_32F_x1(VR_w0,  wBlkExp, VRQ0);
        
                    fmacs(VR_out1, VRQ2, VR_w0, VRQ0, VR_blk, VRQ0, 0);

                    load32x1_vr_postI(VR_blk, pIn0, INC1, VRQ0);
                    load8x1_vr_postI(VR_w0, pWa0, INC1, VRQ0);convert_16I_to_32F_x1(VR_w0, wBlkExp, VRQ0);
    ...
    for (jj = loopGroupLim << 2; jj < m; jj++)  // group
    {
        ...
        if (loopBlockLim2 > 0) {
            ...
            {
                affine_adj_L(VR_out0, VR_w0, VR_blk, 0, 0);
                load_8x4_vr_a(VR_w0, UR_w0, pWa0); convert_16I_to_32F_x4(VR_w0, wBlkExp);
...
static void FullyConnectedQuantizedInt8(
    ...
    const OpDataFullyConnectedEx &data, const int8_t *inputLocal, const int8_t *filterMVM,
...
TfLiteStatus Prepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  OpDataFullyConnectedEx *data_ex = static_cast<OpDataFullyConnectedEx *>(node->user_data);
 ...
 // KN_PRINTX(GetTensorData<float>(bias));
  ...
  if (data_ex->opt_constraint_float)
  {
      ...
      if (filter->type == kTfLiteFloat32)
      {
        ...
        // Allocate tensor weight 
        AScalar* weight_aflt =
            (AScalar*)context->AllocatePersistentBuffer(context, sizeof(AScalar )* weight_size);
...
TfLiteStatus EvalFullyConnectedQuantizedInt8(
    TfLiteContext *context, TfLiteNode *node,const  OpDataFullyConnectedEx &data_ex,
...
TfLiteStatus EvalFloatInt8(TfLiteContext* context, TfLiteNode* node,
    const OpDataFullyConnectedEx& data, TfLiteFusedActivation activation,
...
TfLiteStatus EvalFloat32(TfLiteContext *context, TfLiteNode *node,
  ...
  if (data.opt_constraint_float) {
    ...
    const float* filterLocal =
        (float *)data.weight_aflt; // tflite::micro::GetTensorData<float>(filter);
    const float *biasLocal = bias ? (float *)data.bias_aflt: nullptr;
...
TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node) {
  ...
  TFLITE_DCHECK(node->user_data != nullptr);
  const OpDataFullyConnectedEx& data = *(static_cast<const OpDataFullyConnectedEx*>(node->user_data));
