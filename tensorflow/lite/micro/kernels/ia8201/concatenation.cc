...
#ifdef KN_DEBUG
  // debug overwrite 
  uint16_t* input_offset_start = input_offset;
  uint32_t* input_size_start = input_size;
#endif
  for (int k = 0; k < outer_size; k++) {
    for (int i = 0; i < inputs_count; ++i) {
      ...
      if (output_ptr) {
        ...
        //KN_PRINTD(cp_size);
#ifdef KN_DEBUG
        uint32_t cp_offset32 = (input_ptr - input_data[i]);
        if (cp_offset32 > 0xffffU)
        {
          KN_PRINTS("ERROR!");
          KN_PRINTD(cp_offset32);
        }
#endif
...
#ifdef KN_DEBUG
  size_t isize = input_offset - input_offset_start;
  size_t ssize = input_size - input_size_start;
  KN_PRINTD(isize);
  KN_PRINTD(ssize);
#endif
}
...
int hmd_concat_8_8(const ConCatOpDataEx *data, int8_t *p_out, int32_t num_inp,
...
TfLiteStatus ConcatenationPrepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (output_type) {  // Already know in/outtypes are same.
    ...
    case kTfLiteInt8: {
      ...
      KN_PRINTD(node->inputs->size * sizeof(int32_t));
...
// GetTensorShape(input);
  ...
  data->opt_constraint = 0;
    ...
    inputs_shape[0].DimensionsCount(), data->params.axis);
      */
    int ret = hmd_concat_8_8_prepare(data, output_shape.DimsData(),
      //  inputs_data_int8, 
      inputs_shape_ptr,
