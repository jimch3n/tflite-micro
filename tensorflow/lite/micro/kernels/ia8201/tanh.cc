...
void TanhQuantizedInt8(const OpData* data,  // const AScalar &inputMultipler,
  ...
  if (loopLim > 0) {
    for (int i = 0; i < loopLim; i++) {
      ...
      VR_n0 = vadds(VR_one, VR_z0, 0xf);  // 1- exp
      ...
      rnd_sat_pack(VR_q7_out, VRQ0, VR_y0, 1);
...
void TanhV(float* y, const float* x, int n) {
  ...
  for (int i = 0; i < loopLim; i++) {
    ...
    convert_IEEE_float_to_32F_x4(VR_x1);
    inv(VR_y0, VRQ0, VR_z0, VRQ0);
    inv(VR_y1, VRQ0, VR_z1, VRQ0);
    inv(VR_y0, VRQ1, VR_z0, VRQ1);
    inv(VR_y1, VRQ1, VR_z1, VRQ1);
    inv(VR_y0, VRQ2, VR_z0, VRQ2);
    inv(VR_y1, VRQ2, VR_z1, VRQ2);
    inv(VR_y0, VRQ3, VR_z0, VRQ3);
    inv(VR_y1, VRQ3, VR_z1, VRQ3);
    ...
    // Newton's method
    VR_t1 = vmacs_adj(VR_two, VR_y1, VR_z1, 0xf, 0);
  ...
  if (n & 4) {
    ...
    inv(VR_y0, VRQ3, VR_z0, VRQ3);
    // Newton's method
    VR_n1 = vmacs_adj(VR_two, VR_y0, VR_z0, 0xf, 0);
...
#if defined(DMX1A_TANH_OPT) || defined(HMD1A_TANH_OPT)
