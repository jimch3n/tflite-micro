...
static void SubQuantizedInt8ConstI1(const OpData* data_ex, int8_t* output, const int8_t* input1,
	...
	replicate_ar(vr_offset2, 0xf, data_ex->input2_offset_fr32.fr);
        ...
        replicate_ar(vr_multiplier_input1, 0xf,
                     data_ex->input1_multiplier_fr32.fr);
        replicate_ar(vr_multiplier_input2, 0xf,
                     data_ex->input2_multiplier_fr32.fr);

	replicate_ar(vr_multiplier_output, 0xf,
                     data_ex->output_multiplier_fr32.fr);
	...
	// const 
	vr_shift_input1 =
		vexp_adj(vadds(vr_input1, vr_offset1, 0x0), 31 - 8 - data_ex->SubOp.left_shift);
	...
	if (loopLim > 0)
	{
		...
		// q7*1<<left
		vr_shift_input2 =
			vexp_adj(vadds(vr_input2, vr_offset2, 0x0), 31 - 8 - data_ex->SubOp.left_shift);
	...
	if (n & 3) {
		...
		vr_shift_input2 =
			vexp_adj(vadds(vr_input2, vr_offset2, 0xf0), 31 - 8 - data_ex->SubOp.left_shift);
