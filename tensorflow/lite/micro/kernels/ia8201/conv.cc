  ...
  if (input->type == kTfLiteFloat32) {
...
#else
    bool dilation_xy1 =
        (1 == params.dilation_width_factor &&
    ...
    if (0 != data_ex->opt_constraint_float) {
      ...
      if (filter->type == kTfLiteInt8) {
        ...
        if ((conv2d.padding_x == 0) && (conv2d.padding_y == 0) &&
            (conv2d.stride_x == 1) && (conv2d.stride_y == 1) &&
            (conv2d.ker_x == 1) && (conv2d.ker_y == 1) &&
      ...
      const TfLiteEvalTensor *weightEval =
          tflite::micro::GetEvalInput(context, node, kConvWeightsTensor);
      const float *weight = tflite::micro::GetTensorData<float>(weightEval);
      if (weight) {
        if (filter->type == kTfLiteFloat32)
          tflite::ConvertIEEEFloatToAfloat(weight, (AScalar *)weight,
                                           ElementCount(*weightEval->dims));
...
int ConvFloatKernel(float *y, const float *x, const float *A, const float *bias,
  ...
  if (loopGroupLim > 0) {
    for (jj = 0; jj < loopGroupLim; jj++)  // group
    {
      ...
      pWa3 = pWa2 = pWa1 = pWa0 = (float *)pW;
      load32x1_vr_postR(VR_out0, pWa1, n, VRQ0);
      load32x1_vr_postR(VR_out0, pWa2, n * 2, VRQ0);
      load32x1_vr_postR(VR_out1, pWa3, n * 3, VRQ0);  // dummy load
      ...
      if (loopBlockLim2 > 0) {
        ...
        UR_w0 = align_32x4_load(pWa0);
        UR_w1 = align_32x4_load(pWa1);
        UR_w2 = align_32x4_load(pWa2);
        UR_w3 = align_32x4_load(pWa3);
        UR_blk = align_32x4_load(pIn0);
        load_32x4_vr_a(VR_blk, UR_blk, pIn0);
        convert_IEEE_float_to_32F_x4(VR_blk);
        load_32x4_vr_a(VR_w0, UR_w0, pWa0);
        ...
        for (ii = 0; ii < loopBlockLim2 - 1; ii++)  // group
        {
          ...
          load_32x4_vr_a(VR_w0, UR_w1, pWa1);
          // convert_IEEE_float_to_32F_x4(VR_w0);
          affine_adj_H(VR_out0, VR_w0, VR_blk, 0, 0);
          load_32x4_vr_a(VR_w0, UR_w2, pWa2);
          // convert_IEEE_float_to_32F_x4(VR_w0);
          affine_adj_L(VR_out1, VR_w0, VR_blk, 0, 0);
          load_32x4_vr_a(VR_w0, UR_w3, pWa3);
          // convert_IEEE_float_to_32F_x4(VR_w0);
      ...
      if (n & 3) {
        ...
        for (ii = loopBlockLim2 << 2; ii < n; ii++) {
          ...
          load32x1_vr_postI(VR_w0, pWa2, INC1, VRQ0);
          // convert_IEEE_float_to_32F_x4(VR_w0);
          fmacs(VR_out1, VRQ0, VR_w0, VRQ0, VR_blk, VRQ0, 0);

          load32x1_vr_postI(VR_w0, pWa3, INC1, VRQ0);
          // convert_IEEE_float_to_32F_x4(VR_w0);
...
int ConvApplyOffsetPerCh(
  ...
  vr128 VR_y;
  vr128 VR_b0;
  vr128 VR_outMultPerCh;
  int8_t *pY = outputQ7;
  const int32_t *pB = (const int32_t *)bias;
  ulsr128 UR_Dst = align_32x4_load(pDst);
  ...
  ulsr128 UR_outMultPerCh = align_32x4_load(pOutMultiplerFr32);
  load_32x4_vr_a(VR_outMultPerCh, UR_outMultPerCh, pOutMultiplerFr32);
  load_32x4_vr_a(VR_b0, UR_b, pB);
  load_32x4_vr_a(VR_y, UR_Dst, pDst);

  ulsr128 UR_Y = align_8x4_store(pY);

  uint32_t groupOfChannel = (uint32_t)output_depth >> 2;
  for (uint32_t group = 0; group < groupOfChannel; group++) {
  ...
  if (output_depth & 3) {
    ...
    VR_out = shift8_into32_arith(VR_q7_out, 24, 0, VRQ0);
    for (int32_t j = 0; j < (output_depth & 3); j++) {
...
int ConvQuantizedInt8PerChInputOffset(
  ...
  int loopLimRow = ((m + 7) >> 3);         // group 8 alignment
                                    // FIX for input size pointer is not align 4
                                    // bytes  and  loopLimCol != 0
  ...
  for (int i = 0; i < loopLimRow; i++) {
    ...
    store_32x4_vr_a(VR_y, UR_Dst, pDst);
    VR_y = mov_vr_AccExtend();
    store_32x4_vr_a(VR_y, UR_Dst, pDst);

    flush_32x4(UR_Dst, pDst);
    // store32x4_vr_postI(VR_y, pDst, INC1);
...
#if defined(HMD1A_CONV_OPT) || defined(DMX1A_CONV_OPT)
  ...
  for (int i_out_y = 0; i_out_y < conv2d.out_y; ++i_out_y) {
    for (int i_out_x = 0; i_out_x < conv2d.out_x; ++i_out_x) {
      ...
      tflite::im2col_padding_align4<float>(
          ...
          dim_im_in_x, dim_kernel_x, len_cpy_x, len_cpy_y, ch_im_in, dilation_y,
          dilation_x);
  ...
  for (int i_out_y = 0; i_out_y < conv2d.out_y; ++i_out_y) {
    for (int i_out_x = 0; i_out_x < conv2d.out_x; ++i_out_x) {
      ...
      if (data_ex.opt_constraint == CONV_OPT_TYPE1) {
        tflite::im2col_padding(pBuffer + offset_im_dst,
                               (const int8_t *)input_data + offset_im_src,
                               dim_im_in_x, dim_kernel_x, len_cpy_x, len_cpy_y,
                               ch_im_in, dilation_y, dilation_x);
...
int ConvFloatInt8Kernel(
    float *y, const float *x, const int8_t *A, const float *bias, int m, int n,
    const AScalar &act_min, const AScalar &act_max, const AScalar &scale,
    AScalar *scale_ptr = nullptr  // optional scale is per output channel
) {
  int loopGroupLim = (m >> 2);
  int loopBlockLim2 = (n >> 2);
  //  int jammingBit = RUR_JammingBit();
  ...
  if (loopGroupLim > 0) {
    for (jj = 0; jj < loopGroupLim; jj++)  // group
    {
      ...
      // KN_PRINT_VR128(VR_actOut);
      if (scale_ptr) load32x4_vr_postI(VR_scale, pScalePerCh, INC1);
  ...
  for (jj = loopGroupLim << 2; jj < m; jj++)  // group
  {
    ...
    dsums_L(VR_actOut, VR_out0, 0, 0);
    if (scale_ptr) load32x1_vr_postI(VR_scale, pScalePerCh, INC1, VRQ0);
    VR_actOut = vmuls(VR_actOut, VR_scale, 0);
    // fadds(VR_actOut, VRQ0, VR_out0, VRQ1, VR_out0, VRQ0, 0);
    // VR_exp = afloat_exp_extract(VR_actOut);
    // fmax(VR_maxExp, VRQ0, VR_maxExp, VRQ0, VR_exp, VRQ0);
...
#if defined(HMD1A_CONV_OPT)
int ConvFloatInt8Kernel(float *y, const float *x, const int8_t *A,
                        const float *bias, int m, int n, const AScalar &act_min,
  ...
  if (loopGroupLim > 0) {
    for (jj = 0; jj < loopGroupLim; jj++)  // group
    {
      ...
      if (loopBlockLim2 > 0) {
        ...
        convert_16F_to_32F_x2(VR_w0, TF_FLT16_SIGN, TF_FLT16_EXP,
                              TF_FLT16_BIAS);
        // convert IEEE to afloat
        convert_IEEE_float_to_32F_x2(VR_blk);
        // convert_IEEE_float_to_32F_x2(VR_w0);
        for (ii = 0; ii < loopBlockLim2 - 1; ii++)  // group
        {
          ...
          VR_out0 = vmacs(VR_out0, VR_w0, VR_blk, 0, 0);
          //    KN_PRINT_VR128(VR_out0);
          load16x2_vr_postI_unalign(VR_w0, pWa1, INC1);
          convert_16F_to_32F_x2(VR_w0, TF_FLT16_SIGN, TF_FLT16_EXP,
                                TF_FLT16_BIAS);
          // convert_IEEE_float_to_32F_x2(VR_w0);
          VR_out1 = vmacs(VR_out1, VR_w0, VR_blk, 0, 0);
...

{
  ...
  AScalar *scale_per_channel =
      (AScalar *)data_ex.ConvOp.per_channel_output_multiplier;
  // KN_PRINTD(inFCM);
  // KN_PRINTD(inFCN);
  //         KN_PRINTF(convParam.float_activation_max);
  //      KN_PRINTF(convParam.float_activation_min);
  ...
  for (int i_out_y = 0; i_out_y < conv2d.out_y; ++i_out_y) {
    for (int i_out_x = 0; i_out_x < conv2d.out_x; ++i_out_x) {
      ...
      if (data_ex.opt_constraint_float == CONV_OPT_FLT_X_INT8_PW) {
        ...
        // KN_PRINT_FLOAT(pBuffer, inFCN);
        status = tflite::MVMFloatInt8Kernel(
            &outBuf[outBufIdx * inFCM], (float *)pBuffer,
            (const int32_t *)filter_data, (const float *)bias_data, inFCM,
            inFCN, conv2d.pIm2Col, act_min, act_max, scale, scale_per_channel);
        TFLITE_DCHECK(status == 0);
        outBufIdx += 1;
      } else {
        int offset_im_src, offset_im_dst;
        ...
        // KN_PRINT_FLOAT(pBuffer, inFCN);
        {
          ConvFloatInt8Kernel(&outBuf[outBufIdx * inFCM], (float *)pBuffer,
                              (const int8_t *)filter_data,
                              (const float *)bias_data, inFCM, inFCN, act_min,
                              act_max, scale, scale_per_channel);
          // KN_PRINT_FLOAT(&outBuf[outBufIdx * inFCM], inFCM);
          outBufIdx += 1;
        }
#endif
...
TfLiteStatus EvalConvFloat16Internal(TfLiteContext *context, TfLiteNode *node,
  ...
  if (data_ex.opt_constraint_float) {
    ...
    const float *pInputLocal =
        (const float *)tflite::micro::GetTensorData<float>(input);
    float *pOutputLocal = tflite::micro::GetTensorData<float>(output);
    const TfLiteFloat16 *pFilter =
        tflite::micro::GetTensorData<TfLiteFloat16>(filter);
  ...
  switch (input->type) {  // Already know in/out types are same.
    ...
    case kTfLiteFloat32: {
      switch (filter->type) {
        ...
        case kTfLiteFloat16:
          status =
              EvalConvFloat16Internal(context, node, params, data_ex, input,
                                      filter, bias, output, nullptr);
          break;
        case kTfLiteInt8:
          status = EvalConvFloatInt8(context, node, params, data_ex, input,
                                     filter, bias, output, nullptr);
          break;
        default:
          TF_LITE_KERNEL_LOG(
              context, "filter Type %s (%d) for input float32 not supported.",
...
TfLiteStatus EvalConvFloat32(TfLiteContext *context, TfLiteNode *node) {
  ...
  switch (filter->type) {
    case kTfLiteFloat32:
      ...
      break;
    case kTfLiteFloat16:
      status = EvalConvFloat16Internal(context, node, params, data_ex, input,
                                       filter, bias, output, nullptr);
      break;
    case kTfLiteInt8:
    ...
    default:
      TF_LITE_KERNEL_LOG(
          context,
          "filter Type %s (%d) for float32/float16/int8 not supported.",
          TfLiteTypeGetName(filter->type), filter->type);
      return kTfLiteError;
  }
...
TfLiteStatus EvalConvFloat16(TfLiteContext *context, TfLiteNode *node) {
  ...
  EvalConvFloat16Internal(context, node, params, data_ex, input, filter, bias,
                          output, nullptr);
...
TfLiteStatus EvalConvFloatInt8(TfLiteContext *context, TfLiteNode *node) {
  ...
  EvalConvFloatInt8(context, node, params, data_ex, input, filter, bias, output,
                    nullptr);
