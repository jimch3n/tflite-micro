...
TfLiteStatus EvalInt8(TfLiteContext* context, TfLiteNode* node) {
  ...
  TFLITE_DCHECK(node->user_data != nullptr);
  const OpDataFullyConnectedEx &data_ex = *(static_cast<const OpDataFullyConnectedEx *>(node->user_data));



  status = HmdEvalFullyConnectedQuantizedInt8(context, node, data_ex, input, filter, bias,
                                  output);
  return status;
#endif
...
void *HmdInitFullyConnected(TfLiteContext *context, const char *buffer, size_t length) {
  TFLITE_DCHECK(context->AllocatePersistentBuffer != nullptr);
  return context->AllocatePersistentBuffer(context, sizeof(OpDataFullyConnectedEx));
}

void MVMInputOffsetPrepareWithBias(const int32_t *A, int32_t *output, int m, int n,
                           uint32_t input_offset, const int32_t *bias, int bias_shift)
                       {
  // int32_t *pY = output;
...
int FullyConnectedKernel(int32_t *x, const int32_t *A, const AScalar *bias,
  ...
  KN_PRINTD(signs); 
  KN_PRINTD(output_offset); KN_PRINTX(out_multiplier); KN_PRINTD(out_shift);
...
//KN_PRINTX_VR64(VR_x);KN_PRINTX_VR64(VR_A);
    ...
    for (int32_t j = (loopLimCol << 2); j < nBlockAligned2; j++) {
      load16x1_vr_postI(VR_x, pX, INC1, VRQ0); KN_PRINTX_VR64(VR_x);KN_PRINTX_VR64(VR_A);
      VR_x = vbool(VR_x, VR_inputOffset, 0x6);
   ...
   //  d_out32 = AE_ADD32S(ae_in, d_bias);
    ...
    KN_PRINTX_AE32X2(ae_out0);KN_PRINTX_AE32X2(ae_out1);
  ...
  while (batch) {
    if (data.opt_constraint == 2) {
      status = FullyConnectedKernel(
          ...
          //data.outputOffset, 
          data.input_offset_int8, sign, b_shift,
          op_params.output_offset,
          op_params.output_multiplier, op_params.output_shift);
    } else {
      //KN_PRINTF("ERROR!");
...
TfLiteStatus HmdPrepareFullyConnectedInt8(TfLiteContext *context, TfLiteNode *node) {
 ...
 // KN_PRINTX(GetTensorData<float>(bias));
  ...
  if (data_ex->opt_constraint) {
    ...
    if (true)
#else
    if (use_internal_persist_weight) // intergrate Google Hotword lib to default remap weight
#endif
    {
    ...
    if (data_ex->input_offset_int8 == 0x80808080 ||
      ...
      const int32_t *bias_input =
        ...
        MVMInputOffsetPrepareWithBias(data_ex->mapped_filter, inputOffsetWithW,
                            output_depth, accum_depth,
