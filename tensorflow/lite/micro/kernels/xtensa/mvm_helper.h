...
#define COPY_BLK_BYTES(DST, SRC, BLKCNT)          \
  do {                                            \
    const int kern_x_loop = (BLKCNT) >> 2;        \
    if (kern_x_loop) {                            \
      vr128 vr_im;                                \
      ulsr128 ur_buf2;                            \
      ulsr128 ur_im = align_8x4_load(SRC);        \
      ur_buf2 = align_8x4_store(DST);             \
      for (int ii = 0; ii < kern_x_loop; ii++) {  \
        load_8x4_vr_a(vr_im, ur_im, SRC);         \
        store_8x4_vr_a(vr_im, ur_buf2, DST);      \
      }                                           \
      flush_8x4(ur_buf2, DST);                    \
    }                                             \
                                                  \
    for (int ii = 0; ii < (BLKCNT & 3); ii++) {   \
      vr128 vr_im2;                               \
      load8x1_vr_postI(vr_im2, SRC, INC1, VRQ0);  \
      store8x1_vr_postI(vr_im2, DST, INC1, VRQ0); \
    }                                             \
  } while (0)
...
#define COPY_BLK_BYTES_XOR(DST, SRC, BLKCNT, OFFSET)   \
  do {                                                 \
    const int kern_x_loop = (BLKCNT) >> 2;             \
    vr128 VR_Offset, VR_out;                           \
    replicate_ar(VR_Offset, 0xf, OFFSET);              \
    if (kern_x_loop) {                                 \
      vr128 vr_im;                                     \
      ulsr128 ur_buf2;                                 \
      ulsr128 ur_im = align_8x4_load(SRC);             \
      ...
      if (kern_x_loop > 0) {                           \
        load_8x4_vr_a(vr_im, ur_im, SRC);              \
        for (int ii = 0; ii < kern_x_loop - 1; ii++) { \
          VR_out = vbool(vr_im, VR_Offset, 0x6);       \
          load_8x4_vr_a(vr_im, ur_im, SRC);            \
          store_8x4_vr_a(VR_out, ur_buf2, DST);        \
        }                                              \
        VR_out = vbool(vr_im, VR_Offset, 0x6);         \
        store_8x4_vr_a(VR_out, ur_buf2, DST);          \
        flush_8x4(ur_buf2, DST);                       \
    ...
    for (int ii = 0; ii < (BLKCNT & 3); ii++) {        \
      vr128 vr_im2;                                    \
      load8x1_vr_postI(vr_im2, SRC, INC1, VRQ0);       \
      VR_out = vbool(vr_im2, VR_Offset, 0x6);          \
      store8x1_vr_postI(VR_out, DST, INC1, VRQ0);      \
    }                                                  \
  } while (0)
#define COPY_BLK_BYTES_ALIGN4(DST, SRC, BLKCNT)          \
  do {                                                   \
    ulsr128 UR_src = align_32x4_load(SRC);          \
    ulsr128 UR_des = align_32x4_store(DST);         \
    int loopLim = BLKCNT >> 4;                           \
    if (loopLim > 0) {                                   \
      vr128 VR_S0;                                       \
      load_32x4_vr_a(VR_S0, UR_src, SRC);           \
      for (int32_t ii = 0; ii < (loopLim - 1); ii++) {   \
        store_32x4_vr_a(VR_S0, UR_des, DST);        \
        load_32x4_vr_a(VR_S0, UR_src, SRC);         \
      }                                                  \
      store_32x4_vr_a(VR_S0, UR_des, DST);          \
      flush_32x4(UR_des, DST);                      \
    }                                                    \
    for (int32_t ii = loopLim << 4; ii < BLKCNT; ii++) { \
      vr128 VR_S0;                                       \
      load8x1_vr_postI(VR_S0, SRC, INC1, VRQ0);     \
      store8x1_vr_postI(VR_S0, DST, INC1, VRQ0);    \
    }                                                    \
  } while (false)
#define COPY_BLK_BYTES_XOR_ALIGN4(DST, SRC, BLKCNT, OFFSET) \
  do {                                                      \
    vr128 VR_offset, VR_out;                                \
    ulsr128 UR_src = align_32x4_load(SRC);             \
    ulsr128 UR_des = align_32x4_store(DST);            \
    ...
    if (loopLim > 0) {                                      \
      ...
      for (int32_t ii = 0; ii < (loopLim - 1); ii++) {      \
        ...
        load_32x4_vr_a(VR_S0, UR_src, SRC);            \
        store_32x4_vr_a(VR_out, UR_des, DST);          \
      }                                                     \
      VR_out = vbool(VR_S0, VR_offset, 0x6);                \
      store_32x4_vr_a(VR_out, UR_des, DST);            \
      flush_32x4(UR_des, DST);                         \
...
namespace tflite {
    ...
    void ConvertQ31ToAfloat(const int32_t *input, AScalar *output, int size, int32_t bexp);
...
// int32_t* pShift, int32_t baseExp); //PerShift
    ...
    int DepthWiseConvMap8bitCoeffs(int8_t *pMapped, int32_t mvmMatrix,
                               ...
                               int input_depth);
    void MVMInputOffsetPrepare(const int32_t *A,
	int32_t *output, int m,
	int n,
...
// only able to produce unsigned int8, exceed range is error.
 ...
 static inline void im2col_padding(int8_t* dst, const int8_t* src, int32_t im_x,
        ...
        int32_t len_y, int32_t in_channel, uint16_t dilation_y = 1, uint16_t dilation_x = 1){
    // if(1== in_channel)
        {
#ifdef __XTENSA__
#pragma concurrent
 ...
 static inline void im2col_padding_offset(int8_t* dst, const int8_t* src,
        ...
        int32_t in_channel, uint32_t offset, uint16_t dilation_y =1, uint16_t dilation_x =1){
...
static inline void im2colex_padding_offset_align4(int8_t* dst, int32_t dst_offset,
        const int8_t* src, int32_t src_offset,
        int32_t im_x, int filter_width,
        int32_t len_x, int32_t len_y,
        int32_t in_channel, uint32_t offset, uint16_t dilation_y = 1, uint16_t dilation_x = 1){
    //
    if (1 != dilation_y || 1 != dilation_x)
    {
        ...
        for (int yy = 0; yy < len_y; yy+=dilation_y) {
             ...
             for(int xx = 0; xx < l_wo_ch; xx+=dilation_x)
            {
                ...
                const int8_t* src2 = src+xx * in_channel;
                COPY_BLK_BYTES_XOR_ALIGN4(dst2, src2, in_channel, offset);  // TODO circ Idx
            }
            dst += filter_width * in_channel;
            src += im_x * (dilation_y * in_channel);
        }
...
#pragma concurrent
            ...
            for (int yy = 0; yy < len_y; yy++) {
                // for(int xx = 0; xx < len_x; xx++)
                {
                    int8_t* dst2 = dst;
                    const int8_t* src2 = src;
                    COPY_BLK_BYTES_XOR_ALIGN4(dst2, src2, len_x * in_channel, offset);  // TODO circ Idx
                }
                dst += filter_width * in_channel;
                src += im_x * in_channel;
            }
...
static inline void im2col_padding_unalign(int8_t *dst,int32_t dst_offset, const int8_t *src,
	int32_t src_offset ,
	int32_t im_x, int filter_width,
	int32_t len_x, int32_t len_y,
	int32_t src_channel, int32_t dst_channel, uint32_t offset, uint16_t dilation_y=1, uint16_t dilation_x=1) {
...
static inline void im2col_padding_align4(T *dst, const T *src, int32_t im_x,
                                         int filter_width, int32_t len_x,
                                         int32_t len_y, int32_t in_channel, uint16_t dilation_y=1,
...
static inline void im2col_padding_offset_align4(int8_t* dst, const int8_t* src,
    int32_t im_x, int filter_width,
    int32_t len_x, int32_t len_y,
    int32_t in_channel,
    uint32_t offset, uint16_t dilation_y=1, uint16_t dilation_x=1){
    // if(1== in_channel)
...
static inline void block_fill_words(int32_t *dst, const int32_t filler,
                                    int count) {
  ulsr128 ur_dst = align_32x4_store(dst);
  vr128 src = vseta_vr(0, 0, 0);

  replicate_ar(src, 15, filler);

  for (int i = 0; i < (count >> 2); i++) {
    store_32x4_vr_a(src, ur_dst, dst);
  }
  flush_32x4(ur_dst, dst);
...
static inline void ConvertQ31ToAfloat(const int32_t &input, AScalar &output,
  ...
  output.fr = move32_ar_vr_idx(vr_tmp, VRQ0);
}
#endif
