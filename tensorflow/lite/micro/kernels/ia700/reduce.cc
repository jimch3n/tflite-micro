...
TfLiteStatus PrepareMax(TfLiteContext* context, TfLiteNode* node) {
  ...
  context->RequestScratchBufferInArena(
      context, sizeof(int) * NumElements(output), &op_data->temp_buffer_idx);
  context->RequestScratchBufferInArena(
...
#if defined(HEMILITE_REDUCE_MAX_OPT)
...
TfLiteStatus PrepareMeanOrSum(TfLiteContext* context, TfLiteNode* node) {
  ...
  const TfLiteEvalTensor* axis_eval =
  ...
  for (int ax = 0; ax < op_data->num_axis; ax++) {
    ...
    if (axis_dim >= 0) {
      ...
      num_elements_in_axis *=
          input_shape.Dims(input_shape.DimensionsCount() + axis_dim);
    }
  }
  ...
  if (input->type == kTfLiteFloat32) {
    ...
    op_data->num_output_elements = NumElements(output);
    // ? remove
    context->RequestScratchBufferInArena(context, sizeof(int) * output_size,
                                         &op_data->temp_buffer_idx);
    context->RequestScratchBufferInArena(
        context, sizeof(int) * static_cast<int>(ElementCount(*axis->dims)),
        &op_data->resolved_axis_idx);
...
#if 1
    else if ((output_size & 3) == 0 && 2 == op_data->num_axis &&
...
#if defined(HEMILITE_MEAN_OPT) || defined(HEMILITE_SUM_OPT)
    ...
    while (batches) {
      ...
      if (grpOutput2 > 0)  // FIXME: AND DIM0 == 1, DIM3 = total_output
      {
        ...
        for (int ii = 0; ii < grpOutput2; ii++) {
          ...
          load32x1_vr_postI(VR_x, pInput2, INC1, VRQ1);
          pInput2 += (grpOutput2 - 1) * 2;
...
int ReduceMaxQuantizedInt8(const OpData* data,
  ...
  if (loopLim > 0) {
    ...
    for (int ii = 0; ii < loopLim; ii++) {
      load_32x2_vr_a(VR_max, UR_Dst,
                     pDst);  //, INC1);
                             //	SATURATE_INT32_VR64(VR_max, VR_max,
...
#if defined(HEMILITE_SUM_OPT) || defined(HEMILITE_MEAN_OPT)
