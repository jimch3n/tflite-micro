...
TfLiteStatus SquaredDifferencePrepare(TfLiteContext* context,
...
#if defined(HEMILITE_SQUARED_DIFF_OPT)
  if (output->type == kTfLiteFloat32) {
    ...
    /*if (input1_count == input2_count && input2_count == output_count)
    {
        ...
        data->opt_constraint_float = SQDF_OPT_TYPE2;  // 1: 1,1,3,64 , in2: 64,
    out: 1,3,64
    }
    else */
...

    {
      data->opt_constraint_float =
          SQDF_OPT_TYPE3;  // FloatNulWithScalarBroadcast
    }
  }
#endif
...
#ifdef HEMILITE_SQUARED_DIFF_OPT
...
#endif
TfLiteStatus SquaredDifferenceEvalFloat32(TfLiteContext* context,
                                          TfLiteNode* node) {
  OpData* data = reinterpret_cast<OpData*>(node->user_data);
