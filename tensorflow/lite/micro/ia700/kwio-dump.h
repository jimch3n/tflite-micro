...
#define KN_READ_FP(FILENAME, BUF, DATA_SIZE, ELE_SIZE)                      \
  do {                                                                      \
...
#define KN_WRITE_FP(FILENAME, BUF, DATA_SIZE, ELE_SIZE)  \
  do {                                                   \
...
enum { DMP_HEADER_INFO = 1, DMP_HEX_INFO = 2, DMP_PURE_FLOAT = 4 };
...
#endif
#define KWIO_DUMP_MAX_IO \
  (KN_DUMP_NUM * KN_DUMP_MODE_MAX)  // each componet has 16 i/o

#if defined(KN_DEBUG_DUMP_FILE)  //&& !defined(__XTENSA__) //&& !defined(_DEBUG)
#define KN_DUMP(IDX, FILENAME, BUF, SIZE, DTYPE)                          \
  do {                                                                    \
    int ret = kn_df(IDX, FILENAME, BUF, SIZE, DTYPE, 0, 0xFFFFFFFF, #BUF, \
                    __FUNCTION__, __LINE__);                              \
  } while (0)

#define KN_DUMP2(IDX, FILENAME, BUF, SIZE, AEXP, DTYPE)                      \
  do {                                                                       \
    int ret = kn_df(IDX, FILENAME, BUF, SIZE, DTYPE, AEXP, 0xFFFFFFFF, #BUF, \
                    __FUNCTION__, __LINE__);                                 \
  } while (0)

#define KN_DUMP3(IDX, FILENAME, BUF, SIZE, AEXP, MASK, DTYPE)           \
  do {                                                                  \
    kn_df(IDX, FILENAME, BUF, SIZE, DTYPE, AEXP, 1, #BUF, __FUNCTION__, \
          __LINE__);                                                    \
  } while (0)
#define KN_DUMP4_VR128_COMPLEX(MOD, VR_BUF)                               \
  do {                                                                    \
    VAR_ALIGN_16 ksp_int32_t a##VR_BUF[4];                                \
    store32x4_vr_idxI(VR_BUF, a##VR_BUF, 0);                              \
    kn_df2(MOD, a##VR_BUF, #VR_BUF, 2, 0, KSP_COMPLEX_T, 0, __FUNCTION__, \
           __LINE__);                                                     \
  } while (0)

#define KN_DUMP4(MOD, BUF, SIZE, DTYPE)                                     \
  do {                                                                      \
    kn_df2(MOD, (void *)BUF, #BUF, SIZE, DTYPE, 0, __FUNCTION__, __LINE__); \
  } while (0)

#define KN_DUMP6_CRC(MOD, BUF, ALIAS_BUF, SIZE, DTYPE)                         \
  do {                                                                         \
    void *ALIAS_BUF = (void *)BUF;                                             \
    kn_df2(MOD, (void *)ALIAS_BUF, #ALIAS_BUF, SIZE * sizeof(BUF[0]), KSP_CRC, \
...
#define KN_DUMP6(MOD, BUF, ALIAS_BUF, SIZE, DTYPE)                           \
  do {                                                                       \
    void *ALIAS_BUF = (void *)BUF;                                           \
    kn_df2(MOD, (void *)ALIAS_BUF, #ALIAS_BUF, SIZE, DTYPE, 0, __FUNCTION__, \
           __LINE__);                                                        \
  } while (0)
#define KN_DUMP4_CRC(MOD, BUF, SIZE, DTYPE)                           \
  do {                                                                \
    kn_df2(MOD, (void *)BUF, #BUF, SIZE * sizeof(BUF[0]), KSP_CRC, 0, \
           __FUNCTION__, __LINE__);                                   \
  } while (0)
#define KN_DUMP5(MOD, BUF, SIZE, AEXP, DTYPE)                                  \
  do {                                                                         \
    kn_df2(MOD, (void *)BUF, #BUF, SIZE, DTYPE, AEXP, __FUNCTION__, __LINE__); \
  } while (0)
...
#define KN_PRINT_VR128(VR_IN)                                                  \
  do {                                                                         \
    ...
    printf("%-16s: %d\t0: %.11f\t1: %.11f\t2: %.11f\t3: %.11f\n", #VR_IN,      \
           __LINE__, AScalar(temp_dmp[0]).to_double(),                         \
           AScalar(temp_dmp[1]).to_double(), AScalar(temp_dmp[2]).to_double(), \
           AScalar(temp_dmp[3]).to_double());                                  \
  } while (0)
...
#define KN_PRINT_VR128X(VR_IN, PREFIX)                                      \
  do {                                                                      \
    ...
    printf(                                                                 \
        ...
        "%.10f[0x%08x]\t3: %.10f[0x%08x]\n",                                \
        PREFIX, #VR_IN, __LINE__, tmp_float[0], temp_dmp[0], tmp_float[1],  \
        temp_dmp[1], tmp_float[2], temp_dmp[2], tmp_float[3], temp_dmp[3]); \
  } while (0)
...
#define KN_PRINT_VR128X_IDX(VR_IN, PREFIX, IDX)                       \
  do {                                                                \
    ...
    if (index < 4 && index >= 0) {                                    \
      printf("%-10s %-10s: %d\n%d: %.10f[0x%08x] \n", PREFIX, #VR_IN, \
             __LINE__, IDX, tmp_float[IDX], temp_dmp[IDX]);           \
    }                                                                 \
  } while (0)
...
#define KN_PRINT_VR64X(VR_IN, PREFIX)                                        \
  do {                                                                       \
    ...
    printf("%-10s %-10s: %d\n0: %.10f[0x%08x] \t1: %.10f[0x%08x]\n", PREFIX, \
           #VR_IN, __LINE__, tmp_float[0], temp_dmp[0], tmp_float[1],        \
           temp_dmp[1]);                                                     \
  } while (0)
...
#define KN_PRINT_FR32X(VR_IN, PREFIX)                                       \
  do {                                                                      \
    ...
    convert_32F_to_IEEE_float_x2(VR_tmp2);                                  \
    store32x2_vr_idxI(VR_tmp2, tmp_float, 0);                               \
    printf("%-10s %-10s: %d\n0: %.10f[0x%08x]\n", PREFIX, #VR_IN, __LINE__, \
           tmp_float[0], temp_dmp[0]);                                      \
  } while (0)

#define KN_PRINT_Q15_EXP(I32, EXP)                                   \
  do {                                                               \
    printf("%-8s: %3.6f M: 0x%02x Q: %d %s:%d\n", #I32,              \
           (float)I32 *pow(2.0f, -15 + EXP), I32, EXP, __FUNCTION__, \
           __LINE__);                                                \
  } while (0)
#define KN_PRINT_VR64(VR_IN)                                             \
  do {                                                                   \
    ...
    printf("%-12s: %4d\t0: %g (%08x)\t1: %g (%08x)\n", #VR_IN, __LINE__, \
           temp_dmp[0], *(uint32_t *)&temp_dmp[0], temp_dmp[1],          \
           *(uint32_t *)&temp_dmp[1]);                                   \
  } while (0)
...
#define KN_PRINT(BUF, SIZE, DTYPE)                                      \
  do {                                                                  \
    int ret = kn_df(KWIO_DUMP_MAX_IO, "XXX_out", BUF, SIZE, DTYPE, 0x1, \
                    0xFFFFFFFF, #BUF, __FUNCTION__, __LINE__);          \
  } while (0)
  ...
  do {                                                                   \
    ...
    for (qq = 0; qq < 4; qq++) {                                         \
      printf("%s[%d]: %f M: %x Q: %d %s:%d\n", #VR_X, qq,                \
             (float)tmp_buf[qq] * pow(2.0f, -7 + EXP), tmp_buf[qq], EXP, \
             __FUNCTION__, __LINE__);                                    \
    }                                                                    \
  } while (0)
    ...
    for (i = 0; i < 4; i++) {                                                 \
      ...
      for (j = 0; j < 4; j++) {                                               \
        ...
        printf("%-8s[%-2d]: %3.6f M: 0x%02x Q: %d %s:%d\n", #VR_X, 4 * i + j, \
               (float)tt * pow(2.0f, -7 + EXP), tt & 0xff, EXP, __FUNCTION__, \
               __LINE__);                                                     \
      }                                                                       \
    }                                                                         \
  } while (0)
  ...
  do {                                                                        \
    ...
    for (i = 0; i < 4; i++) {                                                 \
      ...
      for (j = 0; j < 4; j++) {                                               \
        char tt = (char)(tmp8 >> (j * 8));                                    \
        printf("%-8s[%-2d]: %3.6f M: 0x%02x Q: %d %s:%d\n", #VR_X, 4 * i + j, \
               (float)tt * pow(2.0f, -8 + EXP), tt & 0xff, EXP, __FUNCTION__, \
               __LINE__);                                                     \
      }                                                                       \
    }                                                                         \
  } while (0)
//// HIFI AE////
#define KN_PRINT_AE64Q_X(AE_IN, QEXP)                                         \
  do {                                                                        \
    VAR_ALIGN_16 int temp_dmp[4];                                             \
    int64_t INT64 = AE_IN;                                                    \
    double FLT_OUT = powf(2.0f, QEXP) * float(INT64);                         \
    AE_S64_I(AE_IN, (ae_int64 *)temp_dmp, 0);                                 \
    printf("%s: %d\t0: [0x%08x%08x]\t1: %g\n", #AE_IN, __LINE__, temp_dmp[0], \
           temp_dmp[1], FLT_OUT);                                             \
  } while (0)
...
#define KN_PRINTF_IX(A, PREFIX)                                          \
  do {                                                                   \
    fprintf(stdout, "%s : %s %6d (0x%08x) line: %d\n", #A, PREFIX, A, A, \
            __LINE__);                                                   \
  } while (0)
#define KN_PRINTF_I8_SIZE(A, SIZE)                 \
  do {                                             \
    if (A == NULL) break;                          \
...
int get_kn_print();
// void kn_crc(FILE* crc_file, const void *buf, int bytes, const char *buf_name,
// const char *func, const unsigned long line);
...
#pragma comment( \
    lib,         \
    "P:\\AlgoSW\\KSP\\dmx1a\\libs\\kwio-dump\\lib\\DebugDll-Generic-x64\\kwio-dump.lib")
#else
#pragma comment( \
    lib,         \
    "P:\\AlgoSW\\KSP\\dmx1a\\libs\\kwio-dump\\lib\\DebugDll-DeltaMax-Win32\\kwio-dump.lib")
#endif
#endif
#endif
